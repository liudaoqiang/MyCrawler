...
function findCollectionNoCondition(collection) {
    collection.find({"doc_medicine_description": {$regex:/<img src=.*/i}, doc_id: {$gt: 5503}},  { "doc_id": 1, "doc_medicine_description": 1, "doc_medicine_name":1, "_id":0 }).sort({"doc_id": 1}).toArray(function (errfind, cols) {
        if (!errfind) {
            let paths = new Array();
            let i = 0;
            cols.forEath((col)=> {
                const imageEnd = '" style="max-width:100%">';
                const imgStart = '<img src="http://img-doc.chinahanguang.com/yaodian2015/image';
                let str = col.doc_medicine_description;

                let eles = str.split(imgStart);
                eles.forEach((ele) => {
                    let f = ele.split(imageEnd)[0];
                    if (f.endsWith('.jgp')) {
                        paths[i] = f;
                        i++;
                        console.log(f);
